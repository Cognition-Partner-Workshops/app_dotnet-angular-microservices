...
internal static class GoldenFileAssertions
{
    ...
    public static async Task AssertMatchesAsync(HttpResponseMessage response, string fileName)
    {
        ...
        var goldenPath = Path.Combine(AppContext.BaseDirectory, "golden", fileName);

        if (Environment.GetEnvironmentVariable("UPDATE_GOLDEN") == "1")
        {
            var sourcePath = Path.GetFullPath(Path.Combine(AppContext.BaseDirectory, "../../../golden", fileName));
