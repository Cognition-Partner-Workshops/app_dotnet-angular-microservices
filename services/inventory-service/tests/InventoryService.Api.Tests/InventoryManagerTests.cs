...
public class InventoryManagerTests
{
    ...
    [Fact]
    public async Task ReserveStockAsync_DeductsQuantity()
    {
        using var context = CreateContext();
        var service = new InventoryManager(context);
        var result = await service.ReserveStockAsync(1, 10);
        Assert.True(result);
        var item = await service.GetInventoryByProductIdAsync(1);
    ...
    [Fact]
    public async Task ReserveStockAsync_ReturnsFalseForInsufficientStock()
    {
        using var context = CreateContext();
        var service = new InventoryManager(context);
        var result = await service.ReserveStockAsync(2, 100);
        Assert.False(result);
    }
