...
public class InventoryServiceTests
{
    ...
    [Fact]
    public async Task GetByProductId_ReturnsNullForMissingProduct()
    {
        using var context = CreateContext();
        var service = new InventoryItemService(context);
    ...
    [Fact]
    public async Task GetLowStockItems_ReturnsItems_WhenBelowThreshold()
    {
        using var context = CreateContext();
    ...
    public async Task DeductStock_ReturnsNull_WhenInsufficientStock()
    {
        using var context = CreateContext();
        var service = new InventoryItemService(context);
        var item = await service.DeductStockAsync(1, 99999);
        Assert.Null(item);
    }

    [Fact]
    public async Task DeductStock_ReturnsNull_WhenProductMissing()
    {
        using var context = CreateContext();
        var service = new InventoryItemService(context);
