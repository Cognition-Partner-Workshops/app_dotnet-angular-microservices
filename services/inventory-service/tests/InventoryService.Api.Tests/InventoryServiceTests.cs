...
public class InventoryServiceTests
{
    ...
    [Fact]
    public async Task DeductStock_ReturnsTrueAndDeducts()
    {
        using var context = CreateContext();
        var service = new InventoryItemService(context);
        var result = await service.DeductStockAsync(1, 5);
        Assert.True(result);
    ...
    [Fact]
    public async Task DeductStock_ReturnsFalse_WhenInsufficientStock()
    {
        using var context = CreateContext();
        var service = new InventoryItemService(context);
