...
public class InventoryItemService
{
    ...
    public async Task<InventoryItem> CreateOrUpdateInventoryAsync(InventoryItem inventoryItem)
    {
        var existing = await _context.InventoryItems
            .FirstOrDefaultAsync(i => i.ProductId == inventoryItem.ProductId);

        if (existing is not null)
        {
            existing.ProductName = inventoryItem.ProductName;
            existing.Sku = inventoryItem.Sku;
