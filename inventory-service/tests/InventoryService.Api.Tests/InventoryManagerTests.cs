...
public class InventoryManagerTests
{
    ...
    public async Task GetAllInventoryAsync_ReturnsAllItems()
    {
        ...
        var service = new InventoryManager(context);
    ...
    public async Task ReserveStockAsync_DecreasesQuantity()
    {
        using var context = CreateContext();
        var service = new InventoryManager(context);
        var item = await service.ReserveStockAsync(1, 10);
        Assert.Equal(40, item.QuantityOnHand);
    }

    [Fact]
    public async Task ReserveStockAsync_ThrowsOnInsufficientStock()
    {
        using var context = CreateContext();
        var service = new InventoryManager(context);
        await Assert.ThrowsAsync<InvalidOperationException>(() => service.ReserveStockAsync(2, 100));
    }
    ...
    public async Task ReleaseStockAsync_IncreasesQuantity()
    {
        using var context = CreateContext();
        var service = new InventoryManager(context);
        var item = await service.ReleaseStockAsync(1, 10);
        Assert.Equal(60, item.QuantityOnHand);
    }
}
