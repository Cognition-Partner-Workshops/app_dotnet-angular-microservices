...
export class InventoryListComponent implements OnInit {
  ...
  loadInventory() {
    this.http.get<any[]>(`${environment.apiBaseUrl}/inventory`).subscribe(data => this.items = data);
  }

  restock(productId: number) {
    const qty = this.restockQuantities[productId];
    if (!qty || qty <= 0) return;
    this.http.post(`${environment.apiBaseUrl}/inventory/product/${productId}/restock`, { quantity: qty })
      .subscribe(() => {
