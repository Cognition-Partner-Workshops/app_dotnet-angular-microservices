...
export class InventoryListComponent implements OnInit {
  ...
  ngOnInit() { this.load(); }
  load() { this.http.get<any[]>(`${environment.apiUrl}/api/inventory`).subscribe(data => this.items = data.map(i => ({...i, restockQty: 1}))); }
  restock(item: any) {
    this.http.post(`${environment.apiUrl}/api/inventory/product/${item.productId}/restock`, { quantity: item.restockQty || 1 })
      .subscribe(() => this.load());
  }
}
