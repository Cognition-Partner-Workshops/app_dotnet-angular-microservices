...
export class InventoryListComponent implements OnInit {
  ...
  loadAll() { this.http.get<any[]>(this.apiUrl + '/api/inventory').subscribe(data => this.items = data); }
  loadLowStock() { this.http.get<any[]>(this.apiUrl + '/api/inventory/low-stock').subscribe(data => this.items = data); }
  restock(productId: number) {
    this.http.post<any>(this.apiUrl + '/api/inventory/product/' + productId + '/restock', { quantity: this.restockQty })
