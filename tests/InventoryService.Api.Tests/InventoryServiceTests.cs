...
public class InventoryServiceTests
{
    ...
    [Fact]
    public async Task DecrementStockAsync_DecreasesQuantity()
    {
        using var context = CreateContext();
        var service = new InventoryItemService(context);
    ...
    [Fact]
    public async Task DecrementStockAsync_ThrowsWhenInsufficientStock()
    {
        using var context = CreateContext();
        var service = new InventoryItemService(context);
        await Assert.ThrowsAsync<InvalidOperationException>(() => service.DecrementStockAsync(2, 100));
    }
}
