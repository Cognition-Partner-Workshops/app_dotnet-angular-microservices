...
public class InventoryServiceTests
{
    private InventoryDbContext CreateContext()
    {
        ...
        context.InventoryItems.AddRange(
            new InventoryItem { ProductId = 1, ProductName = "Widget A", QuantityOnHand = 50, ReorderLevel = 10, WarehouseLocation = "A1" },
            new InventoryItem { ProductId = 2, ProductName = "Widget B", QuantityOnHand = 5, ReorderLevel = 10, WarehouseLocation = "A2" }
        );
        context.SaveChanges();
        return context;
    }
    ...
    [Fact]
    public async Task DeductStockAsync_ReturnsNull_WhenInsufficient()
    {
        using var context = CreateContext();
        var service = new InventoryItemService(context);
