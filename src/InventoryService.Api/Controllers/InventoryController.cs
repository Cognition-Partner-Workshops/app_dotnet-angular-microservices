...
public class InventoryController : ControllerBase
{
    ...
    public async Task<IActionResult> Update(int productId, [FromBody] UpdateInventoryRequest request)
    {
        try
        {
            ...
            var item = await _inventoryService.UpdateInventoryItemAsync(productId, updated);
            return Ok(item);
        }
        catch (ArgumentException ex)
        {
...
public record CreateInventoryRequest(int ProductId, string ProductName, string Sku, int QuantityOnHand, int ReorderLevel, string WarehouseLocation);
public record UpdateInventoryRequest(string ProductName, string Sku, int QuantityOnHand, int ReorderLevel, string WarehouseLocation);
