...
public class InventoryController : ControllerBase
{
    ...
    [HttpGet("low-stock")]
    [SwaggerOperation(Summary = "Get low stock items", Description = "Returns items where quantity on hand is at or below the reorder level.")]
    [SwaggerResponse(200, "List of low stock items", typeof(List<InventoryItem>))]
    ...
    [SwaggerOperation(Summary = "Check stock availability", Description = "Checks whether the requested quantity is available for a product.")]
    [SwaggerResponse(200, "Stock check result", typeof(StockCheckResponse))]
    public async Task<IActionResult> CheckStock([FromBody] StockCheckRequest request)
    {
        var result = await _inventoryService.CheckStockAsync(request);
        return Ok(result);
    }

    /// <summary>Deduct stock for a product (used by Order service during order creation).</summary>
    [HttpPost("product/{productId:int}/deduct")]
    [SwaggerOperation(Summary = "Deduct stock", Description = "Deducts the specified quantity from a product's inventory. Used by the Order service during order creation.")]
    [SwaggerResponse(200, "The updated inventory item", typeof(InventoryItem))]
    [SwaggerResponse(400, "Insufficient stock or invalid request")]
    [SwaggerResponse(404, "No inventory record for this product")]
    public async Task<IActionResult> DeductStock(int productId, [FromBody] RestockRequest request)
    {
        try
        {
